...
start_stack(){ need_docker; docker compose up -d --build; }
...
recreate_stack(){ need_docker; docker compose up -d --build --force-recreate; }
...
doctor_stack(){ need_docker; echo "== compose =="; docker compose ps || true; echo "== health =="; curl -fsS http://127.0.0.1/health || true; echo; echo "== llama =="; curl -fsS http://127.0.0.1:8081/health || true; echo; echo "== admin =="; curl -fsS http://127.0.0.1:8088/ui/ >/dev/null && echo "admin-ui ok" || true; echo; }
...
Commands:
  ...
  recreate      rebuild and recreate all services
