...
Установщик сохраняет прогресс в `.installer-state.json`. Если установка была
прервана или упала на скачивании модели/Docker images, следующий запуск
`./install.sh install` предложит продолжить с последнего сохранённого шага и
проверит уже готовые файлы.
